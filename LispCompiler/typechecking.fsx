...
// sarebbe stato meglio usare sigleton tagged value
// type envt = Env of Map<string,int> ;;
...
3)  env |- e1 >> n1  e   env |- e2 >> n2   implica   env |-  sum(e1, e2) >> n1 + n2
...
// for enviroments we use againn the class Map, but map vars to exp, not to int
...
(*
 ...
 evaluation: eval : e:exp -> env:enviroment -> exp
...
Alternatively, we could have used a datatype for values

let val = VI of int | VB of bool
*)
...
let less_stupid = evald (Neg (I 3)) Map.empty;;
...
// Note: a FP encoding of rules has to make explcit the *partiality* of the judgment
// first with options
...
// then more informative, with exceptions, to give meaningful errors
...
// a generator for well typed terms
let wellTypedTermsOf =
    Arb.filter (fun x -> tpchko x Map.empty <> None) Arb.from<exp>


// the preservation property: if e : t and  e >> v, then v : t
let prop_pres =
    Prop.forAll wellTypedTermsOf
    <| fun e ->
...
// two type checkers are the same
let prop_tt tenv e=
